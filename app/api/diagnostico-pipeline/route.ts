...
export async function GET(request: NextRequest) {
  const expected = process.env.MARGOTHEIA_BUILD_MARKER;
  ...
  try {
    ...
    if (intakeError || intake?.error || !intake?.intake) {
      return NextResponse.json({ ok: false, stage: "intake", detail: intake?.detail ?? intake?.error ?? intakeError?.message ?? "unknown" });
    }
    ...
    const { data: draw, error: drawError } = await supabase.functions.invoke("start-free-reading", {
      body: { question, refined_question: intake.intake.refined_question, topic: intake.intake.detected_topic, oracle, reversed_enabled: false },
    });
    if (draw?.reading_id) readingId = draw.reading_id;
    if (drawError || draw?.error || !draw?.reading_id) {
      return NextResponse.json({ ok: false, stage: "draw", detail: draw?.detail ?? draw?.error ?? drawError?.message ?? "unknown" });
    }
    ...
    if (readingError || reading?.error || !reading?.interpretation) {
      return NextResponse.json({ ok: false, stage: "interpretation", detail: reading?.detail ?? reading?.error ?? readingError?.message ?? "unknown", code: reading?.code ?? null, cards: Array.isArray(draw.cards) ? draw.cards.map((c: { name: string }) => c.name) : [] });
    }

    return NextResponse.json({ ok: true, stage: "complete", model: reading.model ?? null, prompt_version: reading.prompt_version ?? null, cards: Array.isArray(draw.cards) ? draw.cards.map((c: { name: string }) => c.name) : [], overview_chars: typeof reading.interpretation?.overview === "string" ? reading.interpretation.overview.length : 0 });
  } catch (error) {
